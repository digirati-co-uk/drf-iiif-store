...
def test_public_root_get(http_service):
    """
    Test that the endpoint serving the public 111f3store urls
    is configured.
    :param http_service:
    :return:
    """
    status = 200
    response = requests.get(http_service + "/iiif/")
    assert response.status_code == status


def test_api_root_get(http_service):
    """
    Test that the endpoint serving the api 111f3store urls
    is configured.
    :param http_service:
    :return:
    """
    status = 200
    response = requests.get(http_service + "/api/iiif_store/")
    assert response.status_code == status


def test_api_stored_iiif3_resource_api_create(
    ...
    result = requests.post(
        url=http_service + "/api/iiif_store/resource/",
        json=post_json,
        headers=headers,
        auth=test_api_auth,
    )
    ...
    test_data.manifest_uuid = j.get("id")
...
def test_api_stored_iiif3_resource_api_list(http_service, iiif_store_manifest):
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    result = requests.get(
        url=http_service + "/api/iiif_store/resource/", headers=headers
    )
    j = result.json()
    assert result.status_code == 200
    assert len(j["results"]) == 1
    iiif3_item = j["results"][0]
    assert iiif3_item.get("id") == test_data.manifest_uuid
    assert iiif3_item.get("iiif_type") == "manifest"
    assert iiif3_item.get("original_id") == iiif_store_manifest.get("id")
    assert iiif3_item.get("label") == iiif_store_manifest.get("label")
    assert iiif3_item.get("thumbnail") == iiif_store_manifest.get("thumbnail")
    expected_manifest = copy.deepcopy(iiif_store_manifest)
    expected_manifest.pop("id")
    iiif3_item.get("iiif_json").pop("id")
    assert iiif3_item.get("iiif_json") == expected_manifest


def test_api_stored_iiif3_resource_api_get(http_service, iiif_store_manifest):
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    result = requests.get(
    ...
    assert j.get("iiif_type") == "manifest"
    assert j.get("original_id") == iiif_store_manifest.get("id")
    assert j.get("iiif_json").get("id") != iiif_store_manifest.get("id")
    assert (
        j.get("iiif_json").get("id")
