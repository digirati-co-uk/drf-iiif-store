...
class StoredIIIFResource(TimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        iiif_store_public_url = settings.CANONICAL_HOSTNAME + reverse(
            "iiif_store.public.stored_iiif",
        ...
        if current_id != iiif_store_public_url:
            logger.debug(
                f"Updating iiif id for StoredIIIFResource to public url: ({current_id} -> {iiif_store_public_url})"
            )
